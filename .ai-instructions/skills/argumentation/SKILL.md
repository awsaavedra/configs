...
**Rhetoric vs. philosophy.** Rhetoric is the study of persuasion — how to make arguments land, regardless of truth. Philosophy is the study of truth — what's actually right, then how to make it clear. Sophistry is the corruption of philosophy by rhetoric: arguments that sound rigorous but serve persuasion over truth-seeking. These skills are philosophical. The test is never "does this sound right?" but "is this right?" If you find yourself constructing an argument that would persuade but that you don't actually believe is true, stop — that's the failure mode these skills exist to prevent.
...
**Modes:** Audit (pastes argument → moves found, correctives; if clean, say so); Lookup (describe move → name + one-paragraph corrective); Self-check (silent, address anything that fires).
...
**Skill map:** argumentation-hygiene = rulebook; you-sure = self-audit; steelyman = sparring match; double-crux = symmetric disagreement.
...
**Triggers:** `/you-sure`; "you sure?", "really?", "hold on"; before doubling down reflexively; when user pastes another AI's output to interrogate. If target is ambiguous, pick the most load-bearing claim.
...
3. **Write each rival in its own voice.** Pass the ITT. If you can't, say so explicitly and stop — do not proceed to critique.
...
6. **Revise.** Holds / Holds with caveat / Doesn't hold / Genuinely don't know (don't reach for this lazily — use when the gap is real, not to escape an uncomfortable verdict). End with specific open questions: what remains uncertain, what to check, what assumption is unverified.

**Failure modes to audit in the original claim:** sycophancy; plausibility vs. evidence; recalled vs. checked; conversational priming; authority deference; optimizing for helpfulness-feel over accuracy.

**Cross-AI use:** treat pasted text as the claim, full flow, no softening. Verdict applies to their claim. If already hedged, note it.
...
**Don't use:** routine tasks; user distressed; already-committed decisions; user asked for support. If unsure: "push back or help execute?"
...
2. **Generate 1-2 strongest opposing approaches.** Opposite, simpler, deferred, or reframe.
...
5. **3-5 sharpest questions.** Crux ("what evidence would change your mind?"); surprise ("what would surprise you?"); wager; pre-mortem; boring version; opposite-side test.
6. **Invite response.** "Defend it, revise it, or tell me which question to start with."

**Dialogue:** Good answer → acknowledge + move to next weakness. Dodge → name it. Track which assumptions are defended/open/revised. If the dialogue drifts to bad-faith moves rather than substantive ones, name the pattern from Argumentation Hygiene and offer to reset.
**Credit when:** specific evidence, new context, genuine reframe that dissolves the objection.
**Hold when:** rhetorical, restated more emphatically without new evidence, appealed to authority.
**Calling uncle:** "uncle / enough / ease up / stop" → immediate exit, no last jabs, ask what they need next.
**Final read (3 parts):** what got stronger, what's still weak, what you'd do (recommendation, not verdict — step out of adversarial mode; the goal is the user's proposal surviving stronger, not scoring points).

### Domain Lenses
**Writing:** audience fit; thesis in one sentence; unsaid load-bearing assumption; murder line (what a hostile reader quotes to dismiss); show vs. tell.
...
**Design:** user over self; cognitive load; first-five-minutes; naming; defaults serve the 80% case; boring version.
...
**Don't use:** trivial preferences; performed disagreement; non-load-bearing disagreements.

**Mindset:** Both sides must be willing to update (if not, it's debate — name the asymmetry and offer to switch to Steelyman or You-Sure instead). Cruxes must be real (would you visibly revise if the crux flipped?). Crux-mismatch is progress.

### Flow
1. **Both state position + confidence.** Bet-grade. No softening, no inflating.
2. **Each names their crux.** "I believe X. If [Y] were true, I'd believe not-X." Y = specific, checkable, genuinely load-bearing. Both sides do this before seeing the other's — contaminated cruxes defeat the symmetry.
...
**Fake crux tells:** un-falsifiable ("I'd update if convinced"); motte-and-bailey (narrow crux, broad position — both must flip together); non-load-bearing (would rationalize a new reason to hold same position).
